# -*- coding: utf-8 -*-
"""Fake_Job _Detection.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1aLV2QauasqP2DvzZY32GmbN6K564uL18
"""

import pandas as pd
import matplotlib.pyplot as plt
import numpy as np
#imports the Seaborn library, a Python visualization library built on Matplotlib, for creating informative statistical graphics
...
#command provides summary of  DataFrame, including class type, index range, columns

fake_job_postings.isnull().sum()
#count how many missing values are in each column of the DataFrame.
...
#This block of code extracts state and city information from the location column of
 #the fake_job_postings_US DataFrame and creates a new DataFrame called loc_splitloc_split =[]
loc_split =[]
...
#calculates the proportion of job postings located in the US compared to the total
#number of job postings in the dataset.

fake_job_postings_US = fake_job_postings_US.reset_index()
#resets the row numbers (index) of the DataFrame so they start from 0 and increase by 1.
...
#combines the fake_job_postings_US DataFrame with the loc_split DataFrame by joining them side by side.
...
#creates a new column in the fake_job_postings_US DataFrame called state_city, which combines the values from the state and city columns into a single string, separated by a comma.
...
#remove any leading or trailing spaces from the city and state columns in the fake_job_postings_US DataFrame.
...
#gives you the number of legitimate (non-fraudulent) job postings. and  the number of fake (fraudulent) job postings.
...
# Plots a count plot of the top 10 most frequent state-city combinations, color-coded by fraudulent status.
plt.xticks(rotation=90)
